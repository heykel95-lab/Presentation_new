"""Remove four redundant null-space backups, retaining a restorable archive.

Usage: python3 remove_redundant_backups.py /tmp/nullspace_backup_cleanup
...
REMOVE = {
    31: ('Joint motion: mean of three trials',
         'Additional mean view repeats the main joint-1 comparison. Retained individual trials provide the extra six-setting detail.'),
    33: ('Joint motion: one trial per setting',
         'Every selected repetition is already included in the retained all-individual-trials view.'),
    34: ('Net joint motion over time',
         'Adds a separate net projected-motion metric beyond the cumulative-motion and joint-1 narrative.'),
    35: ('Net joint motion',
         'Endpoint summary repeats the near-zero net-motion takeaway of the removed time-history view.'),
}
...
assert len(entries) == 39 and sum(e['hidden'] for e in entries) == 13
...
parts['docProps/app.xml'] = update_properties(original['docProps/app.xml'], retained, 9, 6)
...
# Standalone archive: original four slide/notes parts, plus all dependencies.
archive = dict(original)
archive['ppt/presentation.xml'], archive['ppt/_rels/presentation.xml.rels'] = select_slide_list(removed)
archive['docProps/app.xml'] = update_properties(original['docProps/app.xml'], removed, 4, 0)
...
with ZipFile(W / 'removed_nullspace_backups_20260922.pptx', 'w', ZIP_DEFLATED) as out:
...
supp = fitz.open()
supp.insert_pdf(pdf, from_page=26, to_page=34)
...
removed_pdf.save(W / 'removed_nullspace_backups_20260922.pdf', garbage=4, deflate=True)

changed_parts = [n for n in parts if parts[n] != original[n]]
allowed_changes = {'ppt/presentation.xml', 'ppt/_rels/presentation.xml.rels',
                   '[Content_Types].xml', 'docProps/app.xml'} | {e['path'] for e in changed_footers}
...
assert len(final) == 35 and len(final_supp) == 9
for i, entry in enumerate(retained):
    ...
    if entry in changed_footers:
        ...
        assert not changed.any(), ('Content outside footer changed', entry['title'])
    else:
...
report = {'date': '2026-09-22', 'removed_slides': removed,
          'retained_nullspace_plot_backups': [e for e in retained if e['physical'] in [29, 30, 31]],
          'main_slides': 26, 'hidden_backups': 9, 'total_slides': 35,
          'supplementary_pages': 9, 'native_equation_count': 29,
          'all_main_slide_xml_and_pdf_pages_unchanged': True,
          ...
          'pdf_archive_contains_four_original_pages': True,
...
print('Staged 35-slide deck and PDF, nine-page supplementary PDF and four-slide archive.')
print('All 26 main slides are unchanged. Retained backup contents differ only in five footer labels.')
