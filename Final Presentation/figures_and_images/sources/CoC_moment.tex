...
% in the direction of r_c: the tool angular offset and force are identical, and
...
% blue for the virtual displacement r_c, red for a force, black for the
...
\begin{scope}
  ...
  \draw[lev] (T) -- (P);
  \node[lbl, above, black] at ($(P)!0.32!(T)+(0,0.04)$) {$r_c$};
  \node[cpt] at (P) {};
  \node[lbl, left, black] at (P) {$p_c$};
  \node[pt] at (T) {};
  ...
  \node[lbl] at ($(T)+(0,1.80)$) {$r_c\times f_n$};

  \node at (0,-0.92) {(a) Supporting moment};
\end{scope}
...
\begin{scope}[xshift=6.0cm]
  ...
  \draw[lev] (T2) -- (P2);
  \node[lbl, above, black] at ($(P2)!0.32!(T2)+(0,0.04)$) {$r_c$};
  \node[cpt] at (P2) {};
  \node[lbl, right, black] at (P2) {$p_c$};
  \node[pt] at (T2) {};
  \node[lbl, anchor=south east] at ($(T2)+(-0.07,0.05)$) {TCP};

  \draw[frc] ($(T2)+(0,1.05)$) -- ($(T2)+(0,0.14)$);
  \node[lbl, right, black] at ($(T2)+(0.72,0.60)$) {$f_n$};

  \coordinate (M2) at ($(T2)+(0,0.30)$);
  \draw[mom] ([shift=(150:0.95)]M2) arc (150:30:0.95);
  \node[lbl] at ($(T2)+(0,1.80)$) {$r_c\times f_n$};

  \node at (0,-0.92) {(b) Opposing moment};
\end{scope}
