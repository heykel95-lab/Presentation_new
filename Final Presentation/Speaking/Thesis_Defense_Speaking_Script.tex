% Figure synchronization 2026-09-10: pitch shaft meets the arrowhead at its base centre. Spoken text and notes remain unchanged.
% Figure synchronization 2026-09-10: pitch and roll arrow tips corrected, yaw preserved.
...
The next slide introduces the centre of compliance, the virtual reference point at which the impedance is defined.
...
\]
The point-shift relation applies locally for small rotations, with all components expressed in the robot base frame.
...
\end{aligned}
\]
CoC displacement generally introduces non-zero off-diagonal blocks in the transformed stiffness matrix \(K_{\mathrm{TCP}}\) and damping matrix \(D_{\mathrm{TCP}}\). Translation and rotation are then coupled: translational error and velocity can contribute to the commanded moment, while rotational error and velocity can contribute to the commanded force.
...
With both controllers introduced, I will now explain the surface-relative settings used in the contact experiment.
...
The next slide introduces the contact experiment and its stiffness settings.
...
\[K_{p,t_1}=K_{p,t_2}=2000\,\mathrm{N/m}.\]
