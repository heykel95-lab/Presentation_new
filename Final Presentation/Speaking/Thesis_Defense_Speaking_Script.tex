...
The Cartesian impedance controller enabled passive contact-induced rotation towards the configured surface. This occurred for both angular-offset directions with the centre of compliance at the TCP. The measured response describes end-effector rotation; it does not independently establish the final physical tool--surface angle.

Higher rotational stiffness reduced that response. Increasing stiffness about $t_1$ from five to fifty newton metres per radian reduced the response by about sixty-four percent. Increasing rotational stiffness was therefore not necessarily beneficial for the investigated contact alignment.

The tested translational-stiffness variation along $t_2$ had little influence: the response span was only zero point zero three degrees. Translational stiffness along $t_1$ was held fixed, so these results do not establish a separate stiffness effect in that direction.

The centre-of-compliance displacement can support or oppose the required rotation. Its additional moment selects a preferred rotational direction; it does not command an alignment trajectory. The TCP-centred configuration, $r_c=0$, is the neutral reference because it adds no CoC-induced moment favouring either direction. The rotational spring and damper remain active. The representative traces also showed that a suitably directed displacement could speed up the response, although its increase in final response was modest.

The null-space controller modified the internal robot configuration while the maximum measured TCP position error remained one point three zero four millimetres, below the two-millimetre criterion. Damping reduced redundant motion; conditioning improved the local configuration indicator and could initiate motion. A small net displacement did not mean that all joint motion was suppressed.

Future work should first use a more rigid tool mount, especially for rotations about $t_2$. The present mount has approximately plus or minus two degrees of unloaded play about the approximately corresponding tool axis, which may contribute to the observed variability.

An adaptive centre of compliance should select a temporary displacement according to the measured entry-tilt direction and return to the TCP after alignment. This strategy was not tested in the current fixed-CoC experiments.

The null-space study should also be extended by testing damping and conditioning together under a physical disturbance, including ways to reduce the back-and-forth motion observed with stronger conditioning.

The main message is that the impedance settings should permit the required contact rotation, and the compliance-centre position should be chosen with respect to its direction. Thank you for your attention. I welcome your questions.
